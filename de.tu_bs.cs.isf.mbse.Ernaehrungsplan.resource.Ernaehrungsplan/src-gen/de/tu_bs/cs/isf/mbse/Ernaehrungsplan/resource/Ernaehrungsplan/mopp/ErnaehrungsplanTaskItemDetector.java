...
/**
 * The ErnaehrungsplanTaskItemDetector is used to find task items in text
 * documents. The current implementation searches for specific keywords to detect
 * task items. The ErnaehrungsplanTaskItemDetector is used both by the
 * TaskItemBuilder and the editor.
 */
public class ErnaehrungsplanTaskItemDetector {
	...
	public List<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan.mopp.ErnaehrungsplanTaskItem> findTaskItems(String text, int line, int charStart) {
		List<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan.mopp.ErnaehrungsplanTaskItem> foundItems = new ArrayList<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan.mopp.ErnaehrungsplanTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan.mopp.ErnaehrungsplanTaskItem(keyword, message, localLine, offset, end));
