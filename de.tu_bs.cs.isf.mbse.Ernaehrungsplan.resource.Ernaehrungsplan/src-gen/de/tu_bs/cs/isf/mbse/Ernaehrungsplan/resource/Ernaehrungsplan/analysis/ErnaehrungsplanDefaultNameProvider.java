...
public class ErnaehrungsplanDefaultNameProvider implements de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan.IErnaehrungsplanNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan.util.ErnaehrungsplanEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
