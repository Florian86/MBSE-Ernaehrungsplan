 ...
 */
public class ErnaehrungsplanPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan";
	/**
	...
	private static ErnaehrungsplanPlugin plugin;
	
	public ErnaehrungsplanPlugin() {
		super();
	}
	...
	public static ErnaehrungsplanPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, ErnaehrungsplanPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, ErnaehrungsplanPlugin.PLUGIN_ID, message);
		}
		final ErnaehrungsplanPlugin pluginInstance = ErnaehrungsplanPlugin.getDefault();
