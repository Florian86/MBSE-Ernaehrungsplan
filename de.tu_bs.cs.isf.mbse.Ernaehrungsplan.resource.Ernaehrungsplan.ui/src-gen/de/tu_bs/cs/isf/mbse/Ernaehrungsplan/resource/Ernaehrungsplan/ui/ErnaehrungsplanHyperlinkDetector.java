 ...
 * hovers, is a proxy.
 */
public class ErnaehrungsplanHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public ErnaehrungsplanHyperlinkDetector(Resource resource) {
		textResource = (de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan.IErnaehrungsplanTextResource) resource;
	}
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.Ernaehrungsplan.ui.ErnaehrungsplanHyperlink(new Region(offset, length), resolvedEObject, text);
