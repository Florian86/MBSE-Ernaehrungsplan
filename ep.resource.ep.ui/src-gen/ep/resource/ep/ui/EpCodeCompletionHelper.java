...
public class EpCodeCompletionHelper {
	...
	private ep.resource.ep.mopp.EpAttributeValueProvider attributeValueProvider = new ep.resource.ep.mopp.EpAttributeValueProvider();
	
	private ep.resource.ep.IEpMetaInformation metaInformation = new ep.resource.ep.mopp.EpMetaInformation();
	 ...
	 * @return
	 */
	public ep.resource.ep.ui.EpCompletionProposal[] computeCompletionProposals(ep.resource.ep.IEpTextResource originalResource, String content, int cursorOffset) {
		...
		ep.resource.ep.IEpTextResource resource = (ep.resource.ep.IEpTextResource) resourceSet.createResource(originalResource.getURI());
		java.io.ByteArrayInputStream inputStream = new java.io.ByteArrayInputStream(content.getBytes());
		ep.resource.ep.IEpMetaInformation metaInformation = resource.getMetaInformation();
		ep.resource.ep.IEpTextParser parser = metaInformation.createParser(inputStream, null);
		ep.resource.ep.mopp.EpExpectedTerminal[] expectedElements = parseToExpectedElements(parser, resource, cursorOffset);
		if (expectedElements == null) {
			return new ep.resource.ep.ui.EpCompletionProposal[0];
		}
		if (expectedElements.length == 0) {
			return new ep.resource.ep.ui.EpCompletionProposal[0];
		}
		java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedAfterCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset));
		java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedBeforeCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset - 1));
		...
		java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> allProposals = new java.util.LinkedHashSet<ep.resource.ep.ui.EpCompletionProposal>();
		java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> rightProposals = deriveProposals(expectedAfterCursor, content, resource, cursorOffset);
		java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> leftProposals = deriveProposals(expectedBeforeCursor, content, resource, cursorOffset - 1);
		...
		for (ep.resource.ep.ui.EpCompletionProposal leftProposal : leftProposals) {
		...
		final java.util.List<ep.resource.ep.ui.EpCompletionProposal> sortedProposals = new java.util.ArrayList<ep.resource.ep.ui.EpCompletionProposal>(allProposals);
		java.util.Collections.sort(sortedProposals);
		org.eclipse.emf.ecore.EObject root = null;
		if (!resource.getContents().isEmpty()) {
			root = resource.getContents().get(0);
		}
		for (ep.resource.ep.ui.EpCompletionProposal proposal : sortedProposals) {
			proposal.setRoot(root);
		}
		return sortedProposals.toArray(new ep.resource.ep.ui.EpCompletionProposal[sortedProposals.size()]);
	}
	
	public ep.resource.ep.mopp.EpExpectedTerminal[] parseToExpectedElements(ep.resource.ep.IEpTextParser parser, ep.resource.ep.IEpTextResource resource, int cursorOffset) {
		final java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedElements = parser.parseToExpectedElements(null, resource, cursorOffset);
		if (expectedElements == null) {
			return new ep.resource.ep.mopp.EpExpectedTerminal[0];
		}
		removeDuplicateEntries(expectedElements);
		removeInvalidEntriesAtEnd(expectedElements);
		return expectedElements.toArray(new ep.resource.ep.mopp.EpExpectedTerminal[expectedElements.size()]);
	}
	 ...
	 * the same position.
	 */
	protected void removeDuplicateEntries(java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedElements) {
		...
		java.util.Map<Integer, java.util.List<ep.resource.ep.mopp.EpExpectedTerminal>> map = new java.util.LinkedHashMap<Integer, java.util.List<ep.resource.ep.mopp.EpExpectedTerminal>>();
		for (int i = 0; i < size; i++) {
			ep.resource.ep.mopp.EpExpectedTerminal elementAtIndex = expectedElements.get(i);
			int start1 = elementAtIndex.getStartExcludingHiddenTokens();
			java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> list = map.get(start1);
			if (list == null) {
				list = new java.util.ArrayList<ep.resource.ep.mopp.EpExpectedTerminal>();
				map.put(start1, list);
			}
		...
		for (int position : map.keySet()) {
			java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> list = map.get(position);
	 ...
	 */
	protected void removeDuplicateEntriesFromBucket(java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedElements) {
		int size = expectedElements.size();
		for (int i = 0; i < size - 1; i++) {
			ep.resource.ep.mopp.EpExpectedTerminal elementAtIndex = expectedElements.get(i);
			ep.resource.ep.IEpExpectedElement terminal = elementAtIndex.getTerminal();
			for (int j = i + 1; j < size;) {
				ep.resource.ep.mopp.EpExpectedTerminal elementAtNext = expectedElements.get(j);
	...
	protected void removeInvalidEntriesAtEnd(java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedElements) {
		for (int i = 0; i < expectedElements.size() - 1;) {
			ep.resource.ep.mopp.EpExpectedTerminal elementAtIndex = expectedElements.get(i);
			ep.resource.ep.mopp.EpExpectedTerminal elementAtNext = expectedElements.get(i + 1);
			...
			// rule.
			ep.resource.ep.grammar.EpSyntaxElement symtaxElementOfThis = elementAtIndex.getTerminal().getSymtaxElement();
			ep.resource.ep.grammar.EpSyntaxElement symtaxElementOfNext = elementAtNext.getTerminal().getSymtaxElement();
	 ...
	 */
	protected void removeKeywordsEndingBeforeIndex(java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> proposals, int index) {
		java.util.List<ep.resource.ep.ui.EpCompletionProposal> toRemove = new java.util.ArrayList<ep.resource.ep.ui.EpCompletionProposal>();
		for (ep.resource.ep.ui.EpCompletionProposal proposal : proposals) {
			ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal = proposal.getExpectedTerminal();
			ep.resource.ep.IEpExpectedElement terminal = expectedTerminal.getTerminal();
			if (terminal instanceof ep.resource.ep.mopp.EpExpectedCsString) {
				ep.resource.ep.mopp.EpExpectedCsString csString = (ep.resource.ep.mopp.EpExpectedCsString) terminal;
	...
	protected String findPrefix(java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedElements, ep.resource.ep.mopp.EpExpectedTerminal expectedAtCursor, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return "";
		}
		int end = 0;
		for (ep.resource.ep.mopp.EpExpectedTerminal expectedElement : expectedElements) {
	...
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> deriveProposals(java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedElements, String content, ep.resource.ep.IEpTextResource resource, int cursorOffset) {
		java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> resultSet = new java.util.LinkedHashSet<ep.resource.ep.ui.EpCompletionProposal>();
		for (ep.resource.ep.mopp.EpExpectedTerminal expectedElement : expectedElements) {
	...
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> deriveProposals(final ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal, String content, ep.resource.ep.IEpTextResource resource, int cursorOffset) {
		ep.resource.ep.IEpExpectedElement expectedElement = (ep.resource.ep.IEpExpectedElement) expectedTerminal.getTerminal();
		if (expectedElement instanceof ep.resource.ep.mopp.EpExpectedCsString) {
			ep.resource.ep.mopp.EpExpectedCsString csString = (ep.resource.ep.mopp.EpExpectedCsString) expectedElement;
			return handleKeyword(expectedTerminal, csString, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof ep.resource.ep.mopp.EpExpectedBooleanTerminal) {
			ep.resource.ep.mopp.EpExpectedBooleanTerminal expectedBooleanTerminal = (ep.resource.ep.mopp.EpExpectedBooleanTerminal) expectedElement;
			return handleBooleanTerminal(expectedTerminal, expectedBooleanTerminal, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof ep.resource.ep.mopp.EpExpectedEnumerationTerminal) {
			ep.resource.ep.mopp.EpExpectedEnumerationTerminal expectedEnumerationTerminal = (ep.resource.ep.mopp.EpExpectedEnumerationTerminal) expectedElement;
			return handleEnumerationTerminal(expectedTerminal, expectedEnumerationTerminal, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof ep.resource.ep.mopp.EpExpectedStructuralFeature) {
			final ep.resource.ep.mopp.EpExpectedStructuralFeature expectedFeature = (ep.resource.ep.mopp.EpExpectedStructuralFeature) expectedElement;
			...
			// completed.
			final java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> proposals = new java.util.ArrayList<ep.resource.ep.ui.EpCompletionProposal>();
	...
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> handleEnumAttribute(ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal, ep.resource.ep.mopp.EpExpectedStructuralFeature expectedFeature, org.eclipse.emf.ecore.EEnum enumType, String prefix, org.eclipse.emf.ecore.EObject container) {
		java.util.Collection<org.eclipse.emf.ecore.EEnumLiteral> enumLiterals = enumType.getELiterals();
		java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> result = new java.util.LinkedHashSet<ep.resource.ep.ui.EpCompletionProposal>();
		for (org.eclipse.emf.ecore.EEnumLiteral literal : enumLiterals) {
			...
			ep.resource.ep.IEpTokenResolverFactory tokenResolverFactory = metaInformation.getTokenResolverFactory();
			ep.resource.ep.IEpTokenResolver tokenResolver = tokenResolverFactory.createTokenResolver(expectedFeature.getTokenName());
			...
			result.add(new ep.resource.ep.ui.EpCompletionProposal(expectedTerminal, resolvedLiteral, prefix, matchesPrefix, expectedFeature.getFeature(), container));
		}
		return result;
	}
	
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> handleNCReference(ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, String prefix, String tokenName) {
		// proposals for non-containment references are derived by calling the reference
		// resolver switch in fuzzy mode.
		ep.resource.ep.IEpReferenceResolverSwitch resolverSwitch = metaInformation.getReferenceResolverSwitch();
		ep.resource.ep.IEpTokenResolverFactory tokenResolverFactory = metaInformation.getTokenResolverFactory();
		ep.resource.ep.IEpReferenceResolveResult<org.eclipse.emf.ecore.EObject> result = new ep.resource.ep.mopp.EpReferenceResolveResult<org.eclipse.emf.ecore.EObject>(true);
		resolverSwitch.resolveFuzzy(prefix, container, reference, 0, result);
		java.util.Collection<ep.resource.ep.IEpReferenceMapping<org.eclipse.emf.ecore.EObject>> mappings = result.getMappings();
		if (mappings != null) {
			java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> resultSet = new java.util.LinkedHashSet<ep.resource.ep.ui.EpCompletionProposal>();
			for (ep.resource.ep.IEpReferenceMapping<org.eclipse.emf.ecore.EObject> mapping : mappings) {
				org.eclipse.swt.graphics.Image image = null;
				if (mapping instanceof ep.resource.ep.mopp.EpElementMapping<?>) {
					ep.resource.ep.mopp.EpElementMapping<?> elementMapping = (ep.resource.ep.mopp.EpElementMapping<?>) mapping;
					...
					resultSet.add(new ep.resource.ep.ui.EpCompletionProposal(expectedTerminal, identifier, prefix, matchesPrefix, reference, container, image));
				}
			}
			return resultSet;
		}
	...
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> handleAttribute(ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal, ep.resource.ep.mopp.EpExpectedStructuralFeature expectedFeature, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EAttribute attribute, String prefix) {
		java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> resultSet = new java.util.LinkedHashSet<ep.resource.ep.ui.EpCompletionProposal>();
				...
				if (defaultValue != null) {
					...
					if (tokenName != null) {
						...
						if (tokenResolver != null) {
							...
							resultSet.add(new ep.resource.ep.ui.EpCompletionProposal(expectedTerminal, defaultValueAsString, prefix, matchesPrefix, expectedFeature.getFeature(), container));
						}
					}
				}
			}
	 ...
	 */
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> handleKeyword(ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal, ep.resource.ep.mopp.EpExpectedCsString csString, String prefix) {
		String proposal = csString.getValue();
		boolean matchesPrefix = matches(proposal, prefix);
		return java.util.Collections.singleton(new ep.resource.ep.ui.EpCompletionProposal(expectedTerminal, proposal, prefix, matchesPrefix, null, null));
	}
	
	/**
	 * Creates a set of (two) completion proposals from the given boolean terminal.
	 */
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> handleBooleanTerminal(ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal, ep.resource.ep.mopp.EpExpectedBooleanTerminal expectedBooleanTerminal, String prefix) {
		java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> result = new java.util.LinkedHashSet<ep.resource.ep.ui.EpCompletionProposal>(2);
		ep.resource.ep.grammar.EpBooleanTerminal booleanTerminal = expectedBooleanTerminal.getBooleanTerminal();
	 ...
	 */
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> handleEnumerationTerminal(ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal, ep.resource.ep.mopp.EpExpectedEnumerationTerminal expectedEnumerationTerminal, String prefix) {
		java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> result = new java.util.LinkedHashSet<ep.resource.ep.ui.EpCompletionProposal>(2);
		ep.resource.ep.grammar.EpEnumerationTerminal enumerationTerminal = expectedEnumerationTerminal.getEnumerationTerminal();
	...
	protected java.util.Collection<ep.resource.ep.ui.EpCompletionProposal> handleLiteral(ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal, org.eclipse.emf.ecore.EAttribute attribute, String prefix, String literal) {
		if ("".equals(literal)) {
			return java.util.Collections.emptySet();
		}
		boolean matchesPrefix = matches(literal, prefix);
		return java.util.Collections.singleton(new ep.resource.ep.ui.EpCompletionProposal(expectedTerminal, literal, prefix, matchesPrefix, null, null));
	}
	 ...
	 * element is expected.
	 */
	protected void setPrefixes(java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedElements, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return;
		}
		for (ep.resource.ep.mopp.EpExpectedTerminal expectedElement : expectedElements) {
	...
	public ep.resource.ep.mopp.EpExpectedTerminal[] getElementsExpectedAt(ep.resource.ep.mopp.EpExpectedTerminal[] allExpectedElements, int cursorOffset) {
		java.util.List<ep.resource.ep.mopp.EpExpectedTerminal> expectedAtCursor = new java.util.ArrayList<ep.resource.ep.mopp.EpExpectedTerminal>();
		for (int i = 0; i < allExpectedElements.length; i++) {
			ep.resource.ep.mopp.EpExpectedTerminal expectedElement = allExpectedElements[i];
		...
		return expectedAtCursor.toArray(new ep.resource.ep.mopp.EpExpectedTerminal[expectedAtCursor.size()]);
	}
	 ...
	 */
	protected int getEnd(ep.resource.ep.mopp.EpExpectedTerminal[] allExpectedElements, int indexInList) {
		ep.resource.ep.mopp.EpExpectedTerminal elementAtIndex = allExpectedElements[indexInList];
		...
		for (int i = indexInList + 1; i < allExpectedElements.length; i++) {
			ep.resource.ep.mopp.EpExpectedTerminal elementAtI = allExpectedElements[i];
	...
	protected boolean matches(String proposal, String prefix) {
		...
		return (proposal.toLowerCase().startsWith(prefix.toLowerCase()) || ep.resource.ep.util.EpStringUtil.matchCamelCase(prefix, proposal) != null) && !proposal.equals(prefix);
	}
	...
	protected org.eclipse.emf.ecore.EObject findCorrectContainer(ep.resource.ep.mopp.EpExpectedTerminal expectedTerminal) {
		...
		ep.resource.ep.grammar.EpContainmentTrace containmentTrace = expectedTerminal.getContainmentTrace();
		org.eclipse.emf.ecore.EClass startClass = containmentTrace.getStartClass();
		ep.resource.ep.mopp.EpContainedFeature currentLink = null;
		ep.resource.ep.mopp.EpContainedFeature previousLink = null;
		ep.resource.ep.mopp.EpContainedFeature[] containedFeatures = containmentTrace.getPath();
			...
			if (hookableParent != null) {
				...
				if (parent != null) {
					if (previousParent == null) {
						...
						ep.resource.ep.mopp.EpContainedFeature link = previousLink;
						ep.resource.ep.util.EpEObjectUtil.setFeature(parent, link.getFeature(), previousParent, false);
					}
				}
			}
		}
		...
		if (parent != null && hookableParent != null) {
			expectedTerminal.setAttachmentCode(new Runnable() {
				...
				public void run() {
					ep.resource.ep.util.EpEObjectUtil.setFeature(finalHookableParent, finalFeature, finalParent, false);
				}
			});
		}
		return correctContainer;
	 ...
	 */
	protected org.eclipse.emf.ecore.EObject findHookParent(org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EClass startClass, ep.resource.ep.mopp.EpContainedFeature currentLink, org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EClass containerClass = currentLink.getContainerClass();
