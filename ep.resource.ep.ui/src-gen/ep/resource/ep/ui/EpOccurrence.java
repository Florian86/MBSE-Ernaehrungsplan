...
public class EpOccurrence {
	...
	private static interface ITokenScannerConstraint {
		public boolean mustStop(ep.resource.ep.ui.IEpTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "ep.resource.ep.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "ep.resource.ep.ui.occurences.declaration";
	
	private final static ep.resource.ep.ui.EpPositionHelper positionHelper = new ep.resource.ep.ui.EpPositionHelper();
	 ...
	 */
	private ep.resource.ep.IEpTextResource textResource;
	 ...
	 */
	public EpOccurrence(ep.resource.ep.IEpTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(ep.resource.ep.IEpLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		ep.resource.ep.ui.IEpTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, ep.resource.ep.ui.EpPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new ep.resource.ep.analysis.EpDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(ep.resource.ep.ui.IEpTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, ep.resource.ep.ui.EpPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, ep.resource.ep.ui.EpPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		ep.resource.ep.ui.IEpTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected ep.resource.ep.ui.IEpTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		ep.resource.ep.IEpLocationMap locationMap = textResource.getLocationMap();
		
		ep.resource.ep.ui.IEpTokenScanner tokenScanner = createTokenScanner();
	...
	protected ep.resource.ep.ui.IEpTokenScanner createTokenScanner() {
		return new ep.resource.ep.ui.EpUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, ep.resource.ep.ui.EpPositionCategory type, String text, int offset, int length) {
		...
		if (type == ep.resource.ep.ui.EpPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(ep.resource.ep.ui.EpOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(ep.resource.ep.ui.EpOccurrence.DECLARATION_ANNOTATION_ID);
	}
