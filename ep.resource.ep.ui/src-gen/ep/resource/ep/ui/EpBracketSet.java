...
public class EpBracketSet {
	...
	private java.util.ArrayList<ep.resource.ep.IEpBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements ep.resource.ep.IEpBracketPair {
	 ...
	 */
	private class ClosingListener implements ep.resource.ep.ui.IEpBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				ep.resource.ep.IEpBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public EpBracketSet(ep.resource.ep.ui.EpEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new ep.resource.ep.mopp.EpMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<ep.resource.ep.IEpBracketPair>();
	 ...
	 */
	public ep.resource.ep.IEpBracketPair getBracketPair(String opening, String closing) {
		for (ep.resource.ep.IEpBracketPair bracketPair : bracketPairs) {
	...
	public ep.resource.ep.IEpBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(ep.resource.ep.IEpBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + ep.resource.ep.ui.EpPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public ep.resource.ep.IEpBracketPair remove(String opening, String closing) {
		for (ep.resource.ep.IEpBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<ep.resource.ep.IEpBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, ep.resource.ep.ui.EpPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, ep.resource.ep.ui.EpPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
