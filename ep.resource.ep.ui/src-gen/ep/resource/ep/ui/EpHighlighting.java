...
public class EpHighlighting implements ISelectionProvider, ISelectionChangedListener {
	 ...
	 */
	private final class UpdateHighlightingListener implements KeyListener, VerifyListener, MouseListener, ep.resource.ep.IEpBackgroundParsingListener {
	 ...
	 */
	public EpHighlighting(ep.resource.ep.IEpTextResource textResource, ProjectionViewer projectionViewer, ep.resource.ep.ui.EpColorManager colorManager, ep.resource.ep.ui.EpEditor editor) {
		this.display = Display.getCurrent();
		projectionViewer.getSelectionProvider();
		this.preferenceStore = ep.resource.ep.ui.EpUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		this.textWidget = projectionViewer.getTextWidget();
		this.projectionViewer = projectionViewer;
		this.occurrence = new ep.resource.ep.ui.EpOccurrence(textResource, projectionViewer);
		this.bracketSet = new ep.resource.ep.ui.EpBracketSet();
		this.colorManager = colorManager;
		this.isHighlightBrackets = preferenceStore.getBoolean(ep.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		this.bracketColor = colorManager.getColor(PreferenceConverter.getColor(preferenceStore, ep.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
	...
	private void setBracketHighlighting(IDocument document) {
		...
		Position[] positions = positionHelper.getPositions(document, ep.resource.ep.ui.EpPositionCategory.BRACKET.toString());
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new ep.resource.ep.ui.EpEObjectSelection(selectedEObject));
				}
			}
		});
	}
	...
	public void resetValues() {
		...
		bracketColor = colorManager.getColor(PreferenceConverter.getColor(preferenceStore, ep.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
