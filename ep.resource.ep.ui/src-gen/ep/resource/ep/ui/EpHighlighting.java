 ...
 */
public class EpHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private ep.resource.ep.ui.EpTokenScanner scanner;
	...
	private ep.resource.ep.ui.EpOccurrence occurrence;
	private ep.resource.ep.ui.EpBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, ep.resource.ep.IEpBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public EpHighlighting(ep.resource.ep.IEpTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, ep.resource.ep.ui.EpColorManager colorManager, ep.resource.ep.ui.EpEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = ep.resource.ep.ui.EpUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new ep.resource.ep.ui.EpTokenScanner(textResource, colorManager);
		occurrence = new ep.resource.ep.ui.EpOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new ep.resource.ep.ui.EpBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(ep.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, ep.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(ep.resource.ep.ui.EpEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, ep.resource.ep.ui.EpPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, ep.resource.ep.ui.EpPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(ep.resource.ep.ui.EpPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new ep.resource.ep.ui.EpEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
