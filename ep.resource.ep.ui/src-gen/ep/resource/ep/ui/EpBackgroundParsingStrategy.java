...
public class EpBackgroundParsingStrategy {
	 ...
	 * Schedules a task for background parsing that will be started after a delay.
	 */
	public void parse(final String contents, final ep.resource.ep.IEpTextResource resource, final ep.resource.ep.ui.EpEditor editor, long delay) {
		if (resource == null) {
			return;
		}
		if (contents == null) {
			...
			while (newContents != null ) {
				while (newContents != null) {
					try {
						...
						if (resource instanceof ep.resource.ep.mopp.EpResource) {
							ep.resource.ep.mopp.EpResource concreteResource = (ep.resource.ep.mopp.EpResource) resource;
