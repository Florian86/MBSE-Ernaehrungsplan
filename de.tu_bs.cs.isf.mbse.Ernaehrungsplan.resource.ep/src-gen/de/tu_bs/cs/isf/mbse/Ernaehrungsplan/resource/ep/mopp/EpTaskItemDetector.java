...
/**
 * The EpTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * EpTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class EpTaskItemDetector {
	...
	public List<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpTaskItem> findTaskItems(String text, int line, int charStart) {
		List<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpTaskItem> foundItems = new ArrayList<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpTaskItem(keyword, message, localLine, offset, end));
