...
public class EpDefaultNameProvider implements de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
