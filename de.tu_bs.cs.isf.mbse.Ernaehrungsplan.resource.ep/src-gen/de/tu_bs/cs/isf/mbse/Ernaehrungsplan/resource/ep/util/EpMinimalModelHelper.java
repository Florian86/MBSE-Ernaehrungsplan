 ...
 */
public class EpMinimalModelHelper {
	
	private final static de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpEClassUtil eClassUtil = new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpListUtil.castListUnchecked(value);
