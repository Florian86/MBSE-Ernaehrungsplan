 ...
 */
public class EpPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep";
	/**
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, EpPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, EpPlugin.PLUGIN_ID, message);
		}
