...
public class EpSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public EpSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
