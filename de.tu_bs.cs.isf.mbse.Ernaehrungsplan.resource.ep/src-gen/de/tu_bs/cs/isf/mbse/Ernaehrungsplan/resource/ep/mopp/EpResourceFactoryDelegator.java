...
public class EpResourceFactoryDelegator implements Factory {
	
	protected Map<String, Factory> factories = null;
	
	public EpResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpRuntimeUtil().isEclipsePlatformAvailable()) {
			new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.util.EpEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpResourceFactory());
		}
	}
