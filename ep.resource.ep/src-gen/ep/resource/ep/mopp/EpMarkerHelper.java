 ...
 * contains <code>ep.resource.ep.IEpTextDiagnostic</code>s it uses the more
...
public class EpMarkerHelper {
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<ep.resource.ep.IEpCommand<Object>> commands = new java.util.ArrayList<ep.resource.ep.IEpCommand<Object>>();
		...
		public void runCommands() {
			java.util.List<ep.resource.ep.IEpCommand<Object>> commandsToProcess = new java.util.ArrayList<ep.resource.ep.IEpCommand<Object>>();
			synchronized(commands) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, ep.resource.ep.IEpTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final ep.resource.ep.IEpTextDiagnostic diagnostic) {
			...
			public boolean execute(Object context) {
				try {
					...
					if (diagnostic instanceof ep.resource.ep.mopp.EpResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((ep.resource.ep.mopp.EpResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<ep.resource.ep.IEpQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, ep.resource.ep.util.EpStringUtil.explode(sourceIDs, "|"));
					}
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, ep.resource.ep.EpEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
