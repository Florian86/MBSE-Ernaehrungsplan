 ...
 */
public class EpResourceBundle {
	
	/**
	 * The name of the main task that is shown in the progress view when the builders
	 * are running.
	 */
	public static String BUILDER_ADAPTER_TASK_NAME = "Building ep file";
	...
	static {
		try {
			ResourceBundle bundle = ResourceBundle.getBundle(EpResourceBundle.class.getName(), Locale.getDefault());
			if (bundle != null) {
				Set<String> keys = bundle.keySet();
				for (String key : keys) {
					String value = bundle.getString(key);
					try {
						Field field = EpResourceBundle.class.getDeclaredField(key.toUpperCase());
