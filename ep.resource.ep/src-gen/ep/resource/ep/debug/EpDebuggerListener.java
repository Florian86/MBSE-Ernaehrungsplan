...
public class EpDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private ep.resource.ep.debug.AbstractEpDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, ep.resource.ep.util.EpPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, ep.resource.ep.util.EpPair<String, Object>>();
	...
	private void runDebugger() throws java.io.IOException {
		...
		ep.resource.ep.debug.EpDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = ep.resource.ep.util.EpStringUtil.encode('#', stack);
				ep.resource.ep.debug.EpDebugMessage message = new ep.resource.ep.debug.EpDebugMessage(ep.resource.ep.debug.EEpDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				ep.resource.ep.debug.EpDebugMessage message = new ep.resource.ep.debug.EpDebugMessage(ep.resource.ep.debug.EEpDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ep.resource.ep.debug.EEpDebugMessageTypes.GET_VARIABLES)) {
				...
				ep.resource.ep.debug.EpDebugMessage message = new ep.resource.ep.debug.EpDebugMessage(ep.resource.ep.debug.EEpDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return ep.resource.ep.util.EpStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		ep.resource.ep.util.EpPair<String, Object> pair = new ep.resource.ep.util.EpPair<String, Object>(name, value);
	...
	public ep.resource.ep.debug.AbstractEpDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(ep.resource.ep.debug.AbstractEpDebuggable debuggable) {
