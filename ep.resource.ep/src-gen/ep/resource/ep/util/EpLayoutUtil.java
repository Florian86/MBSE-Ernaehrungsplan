...
public class EpLayoutUtil {
	...
	public EObject createLayoutInformationModelElement(ep.resource.ep.mopp.EpLayoutInformation layoutInformation, EPackage layoutPackage) {
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), ep.resource.ep.grammar.EpGrammarInformationProvider.getSyntaxElementID(syntaxElement));
	...
	public ep.resource.ep.mopp.EpLayoutInformation createLayoutInformation(EObject layoutInformationModelElement) {
		...
		ep.resource.ep.grammar.EpSyntaxElement syntaxElement = ep.resource.ep.grammar.EpGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof ep.resource.ep.grammar.EpPlaceholder) {
			ep.resource.ep.grammar.EpPlaceholder placeholder = (ep.resource.ep.grammar.EpPlaceholder) syntaxElement;
			ep.resource.ep.IEpTokenResolver tokenResolver = new ep.resource.ep.mopp.EpTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			ep.resource.ep.IEpTokenResolveResult result = new ep.resource.ep.mopp.EpTokenResolveResult();
