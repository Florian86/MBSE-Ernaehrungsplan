...
public class EpBracketSet {
	...
	private ArrayList<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair> bracketPairs;
	...
	public EpBracketSet() {
		...
		this.bracketPairs = new ArrayList<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair>();
	}
	...
	public de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair getBracketPair(String opening, String closing) {
		for (de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair bracketPair : bracketPairs) {
	...
	public boolean addBracketPair(String opening, String closing, boolean closingEnabledInside, boolean closeAfterEnter) {
		...
		bracketPairs.add(new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpBracketPair(opening, closing, closingEnabledInside, closeAfterEnter));
		return true;
	}
	...
	public boolean resetBrackets(IPreferenceStore preferenceStore) {
		String bracketPairs = preferenceStore.getString(languageID + de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair remove(String opening, String closing) {
		for (de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public void deserialize(String bracketSet) {
		bracketPairs = new ArrayList<de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair>();
	...
	public void highlightBrackets(IDocument document, int position, int caretOffset) {
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
