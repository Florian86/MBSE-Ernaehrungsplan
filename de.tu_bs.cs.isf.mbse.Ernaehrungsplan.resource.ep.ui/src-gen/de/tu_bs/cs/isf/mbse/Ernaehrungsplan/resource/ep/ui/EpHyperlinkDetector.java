 ...
 * hovers, is a proxy.
 */
public class EpHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public EpHyperlinkDetector(Resource resource) {
		textResource = (de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpTextResource) resource;
	}
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpHyperlink(new Region(offset, length), resolvedEObject, text);
