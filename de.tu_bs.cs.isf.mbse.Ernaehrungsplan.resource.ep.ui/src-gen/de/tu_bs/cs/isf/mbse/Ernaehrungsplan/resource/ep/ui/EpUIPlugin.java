 ...
 */
public class EpUIPlugin extends AbstractUIPlugin {
	
	public static final String PLUGIN_ID = "de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui";
	public static final String EDITOR_ID = "de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpEditor";
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, EpUIPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, EpUIPlugin.PLUGIN_ID, message);
		}
