 ...
 * </p>
 */
public class EpBracketPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpMetaInformation().getSyntaxName();
	
	private de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public EpBracketPreferencePage() {
		super();
		
		de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpMetaInformation metaInformation = new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.mopp.EpMetaInformation();
	...
	public void init(IWorkbench workbench) {
		setPreferenceStore(de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpBracketSet();
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.deserialize(getPreferenceStore().getString(language + de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
	...
	private void addListenersToStyleButtons() {
		...
		bracketsList.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair bracketPair = getSelectedBracketPair();
				if (bracketPair == null) {
					removeBracketButton.setEnabled(false);
					return;
				}
	...
	protected void performDefaults() {
		...
		enableCheckbox.setSelection(preferenceStore.getDefaultBoolean(de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		matchingBracketsColorEditor.setColorValue(PreferenceConverter.getDefaultColor(preferenceStore, BRACKETS_COLOR));
		String defaultBrackets = preferenceStore.getDefaultString(language + de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	...
	private void updateActiveEditor() {
		...
		preferenceStore.setValue(de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		preferenceStore.setValue(language + de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		IWorkbench workbench = org.eclipse.ui.PlatformUI.getWorkbench();
		IEditorPart editor = workbench.getActiveWorkbenchWindow().getActivePage().getActiveEditor();
		if (editor != null && editor instanceof de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpEditor) {
			((de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.ui.EpEditor) editor).invalidateTextRepresentation();
		}
	}
	
	protected de.tu_bs.cs.isf.mbse.Ernaehrungsplan.resource.ep.IEpBracketPair getSelectedBracketPair() {
